...
**Most recent session**: 2026-05-12 (rounds 1–3 interview completed; all T-01–T-14 re-audited for test specificity and ambiguity; ring buffer semantics, speed formula, tint rendering, lerp removal, and restart mechanic all locked). Git HEAD: see `git log -1`. Lint/tests: not yet run against game code (template skeleton only). Detail: `references/session_handoff_convention.md`.

---

## Parallelization waves

Dependency-ordered groups of tasks that can be implemented in parallel within each wave:

| Wave | Tasks | Notes |
|------|-------|-------|
| 0 | **T-01** | Must be first — all other modules import from `constants.json` |
| 1 | **T-02** | Must come before gameplay modules |
| 2 | **T-03, T-04, T-06, T-11** | All depend only on T-01/T-02; fully independent of each other |
| 3 | **T-07** (needs T-03), **T-05** (needs T-04) | Can run in parallel with each other |
| 4 | **T-08, T-09** | Both need T-03 + T-06 (wave 2) and T-07 (wave 3); can run in parallel |
| 5 | **T-10** (needs T-09, T-04, T-11), **T-12** (needs T-03, T-08, T-09) | Parallel |
| 6 | **T-13** (needs T-10, T-12), **T-14** (needs T-11, T-12) | Parallel; final integration wave |

---

## T-01 — Create `src/constants.json` `@next`
...
## T-02 — Scaffold main p5 sketch (`src/main.ts`)
...
## T-04 — Voronoi zone calculator (`src/voronoi.ts`)
...
## T-05 — Zone renderer (`src/zone-renderer.ts`)
...
## T-06 — Asteroid system (`src/asteroid.ts`)
...
## T-07 — Bullet system (`src/bullet.ts`)
...
## T-08 — Collision detection (`src/collision.ts`)
...
## T-09 — Zone undo tree (`src/zone-history.ts`)
...
## T-10 — Spinner time-travel controller (`src/time-travel.ts`)
...
## T-11 — Energy system (`src/energy.ts`)
...
## T-12 — Win condition / game state machine (`src/game.ts`)
...
## T-13 — Input wiring (`src/input.ts`)
